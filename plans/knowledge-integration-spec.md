...
- Publishing into shared AD4M perspectives. v1 lives entirely in a private perspective on the local AD4M node.
...
3. **Multi-machine.** v1 assumes a single host. When Sovereign grows to a second machine the right answer is publishing into a shared AD4M perspective and reading from both — but the federation story is a separate spec.
