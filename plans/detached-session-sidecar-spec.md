...
Probed on Arcadia against Claude Code 2.1.178 / SDK 0.3.220 (2026-07-27):
...
**2. The in-process MCP server probably can't come along.** Sovereign registers its MCP server as `McpSdkServerConfigWithInstance` — a live JS object, not a subprocess. A detached worker is a separate process and cannot hold a reference to it. Sovereign's own tools would have to move to the HTTP sidecar (`SOVEREIGN_MCP_HTTP_URL`, already exists) for every session. Doable, but it's a second migration riding on the first.
