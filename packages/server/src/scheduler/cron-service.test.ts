...
import type { AgentBackend, BackendCapabilities } from '@sovereign/core'
import { createScheduler } from './scheduler.js'
import { createCronService } from './cron-service.js'
import { createBackend, type RoutingBackend } from '../agent-backend/factory.js'
import { createSessionsRegistry } from '../agent-backend/shared/sessions-registry.js'
import { createBackendEmitter } from '../agent-backend/shared/event-emitter.js'
...
describe('createCronService — Sovereign-native user-message cron', () => {
  ...
  beforeEach(() => {
    ...
    routing = createBackend({
      enabled: ['claude-code'],
      default: 'claude-code',
      registry,
      factories: { 'claude-code': () => makeStubBackend([]) }
    })
  })
  ...
  it('routes a fired cron job into the bound thread via routing.forSession', async () => {
    ...
    routing = createBackend({
      enabled: ['claude-code'],
      default: 'claude-code',
      registry: createSessionsRegistry(dataDir, { debounceMs: 0 }),
      factories: { 'claude-code': () => stub }
    })
    const service = createCronService({ routing, scheduler, bus })
