  ...
  it('MUST translate OpenClaw streaming messages to chat.stream events', async () => {
    ...
    serverWs.send(
      JSON.stringify({
        type: 'event',
        event: 'chat',
        payload: { state: 'delta', message: [{ type: 'text', text: 'hello' }], sessionKey: 'main' }
      })
    )
  ...
  it('MUST translate OpenClaw turn completion to chat.turn events with a fully parsed ParsedTurn', async () => {
    ...
    serverWs.send(
      JSON.stringify({
        type: 'event',
        event: 'chat',
        payload: { state: 'final', message: [{ type: 'text', text: 'hi' }], sessionKey: 'main' }
      })
    )
  ...
  it('MUST translate OpenClaw tool call/result messages to chat.work events', async () => {
    ...
    serverWs.send(
      JSON.stringify({
        ...
        payload: {
          stream: 'tool',
          data: { name: 'read', input: '{}', phase: 'call', timestamp: 1000 },
          sessionKey: 'main'
        }
      })
    )
  ...
  it('MUST translate OpenClaw thinking block messages to chat.work events with type thinking', async () => {
    ...
    serverWs.send(
      JSON.stringify({
        type: 'event',
        event: 'agent',
        payload: { stream: 'thinking', data: { text: 'reasoning...' }, sessionKey: 'main' }
      })
    )
    const w = (await p).work
  ...
  it('MUST translate OpenClaw status transitions to chat.status events', async () => {
    ...
    serverWs.send(
      JSON.stringify({
        type: 'event',
        event: 'agent',
        payload: { stream: 'lifecycle', data: { phase: 'start' }, sessionKey: 'main' }
      })
    )
  ...
  it('MUST translate OpenClaw compaction messages to chat.compacting events', async () => {
    ...
    serverWs.send(
      JSON.stringify({
        type: 'event',
        event: 'agent',
        payload: { stream: 'compaction', data: { phase: 'start' }, sessionKey: 'main' }
      })
    )
  ...
  it('MUST translate OpenClaw error messages to chat.error events', async () => {
    ...
    serverWs.send(
      JSON.stringify({
        type: 'event',
        event: 'agent',
        payload: { stream: 'lifecycle', data: { phase: 'error', error: 'fail' }, sessionKey: 'main' }
      })
    )
  ...
  it('MUST strip thinking blocks from streamed text', async () => {
    ...
    serverWs.send(
      JSON.stringify({
        ...
        payload: {
          state: 'delta',
          message: [{ type: 'text', text: 'hello <think>secret</think> world' }],
          sessionKey: 'main'
        }
      })
    )
  ...
  it('MUST preserve code blocks even if they contain thinking-like tags', async () => {
    ...
    serverWs.send(
      JSON.stringify({
        type: 'event',
        event: 'chat',
        payload: { state: 'delta', message: [{ type: 'text', text }], sessionKey: 'main' }
      })
    )
  ...
  it('MUST emit chat.error with retryAfterMs set to the gateway indicated retry delay', async () => {
    ...
    serverWs.send(
      JSON.stringify({
        ...
        payload: {
          stream: 'lifecycle',
          data: { phase: 'error', error: 'rate limited', retryAfterMs: 5000 },
          sessionKey: 'main'
        }
      })
    )
  ...
  it('MUST automatically retry the request after the indicated delay', async () => {
    ...
    serverWs.send(
      JSON.stringify({
        type: 'event',
        event: 'agent',
        payload: { stream: 'lifecycle', data: { phase: 'error', error: 'rate limited' }, sessionKey: 'main' }
      })
    )
    await waitFor(100)
